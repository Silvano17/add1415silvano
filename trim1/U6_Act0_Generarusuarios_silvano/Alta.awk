...
BEGIN{
	...
	dominio="OU=Formacion Profesional,DC=perez,DC=edu";
...
}
{
	...
	print "dsadd user -upn "usuario"@perez.edu \"CN="nombreordenado","unidad"\" -samid "usuario" -disabled no -pwd $M"clave"a# -mustchpwd yes -memberof \""grupo"\" -ln \""apellido1" "apellido2"\" -display \""nombrecompleto"\" -q" > fichero
	print "echo "nombreordenado"\t[OK]" > fichero
	carpeta=home"\\"direc"\\"usuario
	print "md "carpeta > fichero
	print "cscript.exe xcacls "carpeta" /Q /G CLH\\"usuario":f Administrators:f "profesores > fichero
}
END{
	print "cscript.exe xcacls "home"\\PCON /Q /G CLH\\PCON:x Administrators:f" > fichero
	print "cscript.exe xcacls "home"\\PINF /Q /G CLH\\PINF:x Administrators:f" > fichero
	print "cscript.exe xcacls "home"\\1INF /Q /G CLH\\1INF:x Administrators:f" > fichero
	print "cscript.exe xcacls "home"\\2INF /Q /G CLH\\2INF:x Administrators:f" > fichero
	print "cscript.exe xcacls "home"\\1CON /Q /G CLH\\1INF:x Administrators:f" > fichero
	print "cscript.exe xcacls "home"\\2CON /Q /G CLH\\2INF:x Administrators:f" > fichero
}	
